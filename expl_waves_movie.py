...
Propagation_time = 600                                    # Propagation time and seismogram length (s)

# phases_to_plot = ['P','Pdiff','PKP','PKIKP', 'PcP', 'PKKP', 'PP']
# phases_to_plot = ['P', 'PKiKP', 'PP', 'PcP', 'Pn']
# color_attenuation=[1.0,  0.5,    1.0,  0.3,   0.3]
...
max_amp    = round(np.max(seis_data))
min_amp    = round(np.min(seis_data))
...
# Create buffer arrays - width of TW_duration
...
plt.xlabel('Time before present (s)', fontsize=10)
...
# Present time - TW_duration - present time
