...
epi_dist = 50                             # Epicentral distance from Earthquake to Station - use station longitude to increase this 0-180 allowed

depth_earthquake = 0                    # depth of earthquake in km

Propagation_time = 650                  # Propagation time and seismogram length (s) - up to 3600s
...
# ax1.text(0, min_amp+0.05, 'Earthquake waves arriving', ha="left", va="bottom",fontsize=12, color='black', bbox=dict(facecolor='white', edgecolor='white', pad=1.0))
...
def animate(t, lines_left, lines_right):
    ...
    # # Flashing ray path
    # if t % 20 < 10 or t > key_phase_A_time:
    #     key_ray_path.set_data(key_path[0,0:t],radius - key_path[1,0:t])
    #     key_ray_path.set_color('r')
    # else:
    #     key_ray_path.set_data(key_path[0,0:t],radius - key_path[1,0:t])
    #     key_ray_path.set_color('w')
    # Constant ray motion
    key_ray_path.set_data(key_path[0,0:t],radius - key_path[1,0:t])
    ...
    #     ax1.text(0, min_amp+0.05, 'Earthquake waves arriving '+str(waiting)+str(waiting_gap), ha="left", va="bottom",fontsize=12, color='black', bbox=dict(facecolor='white', edgecolor='white', pad=1.0))
    # else:
    #     ax1.text(0, min_amp+0.05, 'Waves arrived!                               ', ha="left", va="bottom",fontsize=12, color='black', bbox=dict(facecolor='white', edgecolor='white', pad=1.0))
    #
    # # Dynamic x-tick labelling.
    # plt.xlabel('Time after Earthquake (min)', fontsize=12)
    ...
    return(line,eq_symbol,seismom_symbol,key_ray_path)
